...
export default async function handler(req: any, res: any) {
  ...
  ・適切な改行を入れること
  ・2〜4文ごと、または話題が変わるごとに必ず改行する
  ・1つの段落を長くしすぎない
  ・仕事内容、勤務条件、給与、福利厚生は、読みやすいところで改行する
  ・箇条書きに向く情報は「・」を使って整理してよい
  ・同じ内容を言い換えて繰り返さない
  ・求人票全体は情報量を保ちつつ、冗長にならないよう簡潔にまとめる
  ・入力された条件を勝手に変更しない
  ...
  ・description は長すぎないよう、原則350〜600文字程度を目安にする
  ...
  ・title：25〜50文字程度。職種名と主要な魅力を簡潔にまとめる
  ・catchCopy：35〜70文字程度。具体的な条件やメリットを1〜2個入れる
  ・description：350〜600文字程度。仕事内容が具体的に分かる十分な量にする
  ・requirements：150〜300文字程度。応募条件と向いている人物像を簡潔に説明する
  ・salary：80〜180文字程度。給与額、昇給、賞与、試用期間など入力済み情報を簡潔に整理する
  ・workingHours：120〜250文字程度。勤務時間、シフト、休日、残業などを読みやすく整理する
  ・location：60〜140文字程度。住所、最寄り駅、徒歩分数を簡潔にまとめる
  ・benefits：入力されている内容を3〜7項目程度に整理する。入力数が少ない場合は無理に増やさない
  ・appealPoints：3〜5項目程度。各項目は1〜2文程度で簡潔に説明する
  ...
  description：
  仕事内容。原則350〜600文字程度を目安にし、応募者が実際の仕事を具体的に想像できる文章にする
  
  requirements：
  応募資格・求める人物像。原則150〜300文字程度を目安にし、入力済み条件とターゲットに合わせて簡潔に説明する
  
  salary：
  給与。原則80〜180文字程度を目安にし、給与額・昇給・賞与・試用期間等を簡潔に整理する
  
  workingHours：
  勤務時間・休日・残業など。原則120〜250文字程度を目安にし、シフトや最低勤務日数・時間等も読みやすく整理する
