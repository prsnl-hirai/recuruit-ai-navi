...
・入力された求人条件を勝手に変更しない
・存在しない待遇、資格、勤務条件などを追加しない
・給与を勝手に変更しない
・勤務時間を勝手に変更しない
・仕事内容の記載が少ない場合、業種と職種から推測される仕事内容を補足すること
・AIやシステム側の事情を求人票に書かない
・「現時点で指定されていません」「未指定です」「情報がありません」「記載がありません」などの表現を求人票に使用しない
・応募資格が入力されていない場合は、求人票の「requirements」を空欄にするか、入力されている仕事内容や雇用条件から自然に判断できる内容だけを記載する
・「特になし」「未指定」「不明」などのシステム的な表現を求人票に表示しない
・求人票は実際の求人媒体に掲載する文章として自然に作成する
・AIが求人を生成していることを感じさせる表現を使用しない
・「AIによる分析では」「AIの推定では」「AIが判断した結果」などの表現を求人票に使用しない
...
・勤務地から最寄り駅を判断する
・最寄り駅が判断できる場合は、駅名と路線名を自然に記載する
・最寄り駅の表示は「○○線○○駅」のように簡潔にする
・「○○駅が候補です」「○○駅が候補となります」「○○駅と考えられます」「○○駅が推定されます」などの表現は使用しない
・「最寄り駅候補」「推定最寄り駅」などの見出しも使用しない
・徒歩分数について確実な情報がない場合は、徒歩分数を記載しない
・駅を正確に判断できない場合は、無理に駅名を生成せず空配列にする
...
totalは上記4項目を総合して判断してください。

【タイトル】

待遇や立地の良さ、シフトの柔軟さなど、求人の魅力をアピールできるようなタイトルをつけてください。

【改善提案】

応募を増やすために、
効果が大きいと思われる順に最大3件提案してください。
...
給与水準は、勤務地を鑑みて水準を判断してください。

あまり長文で書かず、要点のみを端的に記載してください。

タイトルについては提案しないでください。

【応募資格】

応募資格が入力されていない場合、無理に「応募資格はありません」などとは書かないでください。

仕事内容や雇用条件から自然に判断できる内容がある場合のみ記載してください。

判断できる内容がない場合は空文字（""）にしてください。
